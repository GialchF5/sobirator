...
Синхронизация данных между несколькими Google Sheets без ручного IMPORTRANGE: функция переносит нужные диапазоны по расписанию.

## Назначение

exportLeadsScript: Скрипт обмена данными между несколькими Google Sheest, скриптовый аналог Importrange(). 

## Параметры функции

- ID функции: `d4e38gral0su1lpkk60i`
- Каталог Yandex Cloud: `sl`
- Статус: `ACTIVE`
- Runtime: `nodejs22`
- Entry point: `index.handler`
- Версий в экспорте: `4`
- HTTP URL: `https://functions.yandexcloud.net/d4e38gral0su1lpkk60i`

## Триггеры

- `export-leads` (`a1sst4me8a2afd0jiuah`), статус: `ACTIVE`, cron: `*/30 * ? * * *`
...
```powershell
cd .\yc-export-author-gilach\sanitized\functions\sobirator
...
```powershell
yc serverless function version create --function-id d4e38gral0su1lpkk60i --runtime nodejs22 --entrypoint index.handler --source-path . --execution-timeout 60s
```
